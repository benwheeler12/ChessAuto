...
- **Prototype 4 — open board, opponent first**: like prototype 3 but nothing
  ...
  and rooks.
- **Prototype 5 — discovery set**: identical rules to prototype 4, but the
  puzzles come from a different discovery algorithm: feature-outlier
  positions from real Lichess games (see *Puzzle lab* below), qualified by
  the same engine verification. Each puzzle's description names the
  heuristics that discovered it, so playtest impressions map back to
  features. Generated by `node scripts/generate-p5.mjs`.
