...
for (const puzzle of puzzles) {
  ...
  if (puzzle.p3 || puzzle.p4) {
    const blocked = puzzle.p4 ? [] : puzzle.p3.excluded;
    const winners = puzzle.p4 ? puzzle.p4.solutions : [puzzle.p3.solution];
...
for (const puzzle of puzzles) {
  ...
  for (const [key, r] of results) {
    ...
    if (mode === 'own') {
      ...
      if (puzzle.p4?.solutions.includes(sq) && !r.playerWon) {
        puzzle.p4.solutions = puzzle.p4.solutions.filter((s) => s !== sq);
        if (!puzzle.p4.solutions.length) { console.error(`    no P4 solutions left — dropping P4 for ${puzzle.id}`); delete puzzle.p4; }
      } else if (r.playerWon && puzzle.p4 && !puzzle.p4.solutions.includes(sq)) {
        if (puzzle.p4.solutions.length < 2) puzzle.p4.solutions = [...puzzle.p4.solutions, sq].sort();
        else { console.error(`    third winning square found — dropping P4 for ${puzzle.id}`); delete puzzle.p4; }
      }
      if (puzzle.p3) {
...
writeFileSync(FILE, `${banner}export default ${JSON.stringify(puzzles)};\n`);
